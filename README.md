...
A modern, Spotify-themed podcast streaming platform for technology education. Built with vanilla HTML, CSS, and JavaScript.
...
- **🔄 Auto-Updates**: Automatically fetches latest episodes from Spotify RSS feed
...
- **🛡️ Fallback System**: Shows cached episodes if API is temporarily unavailable
...
**Your site now automatically fetches the latest episodes from Spotify!** 

When you upload a new podcast to Spotify, your website will automatically update within minutes. No manual code changes needed!

#### How It Works:
1. **Upload** your podcast to Spotify as usual
2. **Wait** a few minutes for Spotify's RSS feed to update
3. **Refresh** your website - new episodes appear automatically!

The site fetches episodes from your Spotify show's RSS feed using the show ID: `2SBB7iZaCa0wekK0dSvHDT`

#### Fallback System:
- If the RSS feed is temporarily unavailable, the site shows fallback episodes
- Users always see content, even if the API is down
- Check browser console for loading status messages

#### To Change the Spotify Show:
Edit the show ID in [script.js](script.js):
```javascript
const SPOTIFY_SHOW_ID = 'your-show-id-here';
```

### Adding Episodes Manually (Optional)

If you prefer manual control, edit the `fallbackEpisodes` array in [script.js](script.js):
